...
public:
	...
	TArray() : vector{} {}
	TArray(TArray<Type>&& other) { operator=(std::move(other)); }
	...
	int32 Delete(const Type& item) {
		int32 lastSize = Size();
		vector.erase(std::remove_if(vector.begin(), vector.end(), item), v.end());
	 ...
	 * @return number of deleted items
	 */
	int32 DeleteWhere(std::function<bool(const Type&)>&& callback) {
		int32 lastSize = Size();
		vector.erase(std::remove_if(v.begin(), v.end(), callback), v.end());
		return lastSize - Size();
	}
